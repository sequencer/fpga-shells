...
class XilinxKU040MIGPads(depth : BigInt) extends KU040MIGIODDR(depth) {
  def this(c : XilinxKU040MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxKU040MIGIO(depth : BigInt) extends KU040MIGIODDR(depth) with KU040MIGIOClocksReset

class XilinxKU040MIGIsland(c : XilinxKU040MIGParams)(implicit p: Parameters) extends LazyModule with HasCrossing {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x100000000L),"ku040mig supports upto 4GB depth configuraton")
  ...
  lazy val module = new LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxKU040MIGIO(depth)
    })

    //MIG black box instantiation
    val blackbox = Module(new ku040mig(depth))
